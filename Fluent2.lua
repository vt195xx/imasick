...
InstanceMeta.__index = function(self, key)
	local data = InstanceData[self]
	if key == "Name" then return data.Name end
	if key == "Parent" then return data.Parent end
	if key == "ClassName" then return data.Kind end
	if key == "GetChildren" then
		return function(s)
			local list = {}
			for _, c in pairs(InstanceData[s].Children) do
				table.insert(list, c)
			end
			return list
		end
	...
	if key == "FindFirstChild" then
		return function(s, n)
			return InstanceData[s].Children[n]
		end
	end
	local child = data.Children[key]
	if child ~= nil then
		return child
	end
	return nil
...
local function NewNode(name, kind, parent, loader)
	local inst = setmetatable({}, InstanceMeta)
	InstanceData[inst] = { Name = name, Kind = kind, Parent = parent, Children = {}, Loader = loader, Started = false, Cache = nil }
	if parent then
		InstanceData[parent].Children[name] = inst
	end
	return inst
end

local function require(inst)
	local data = InstanceData[inst]
	if not data.Started then
		data.Started = true
		assert(data.Loader, "Attempted to require a Folder (not a ModuleScript): " .. tostring(data.Name))
		data.Cache = data.Loader(inst)
	end
	return data.Cache
end

local N1 = NewNode('MainModule', "ModuleScript", nil, function(script)
...
function Library:CreateWindow(Config)
	...
	-- No more splash/welcome screen. The window already has its own
	-- entrance scale-bounce (see EntranceMotor in Window.lua) - we just
	-- additionally fade its contents in from transparent so it doesn't
	-- pop in as a flat, un-animated block.
...
local N2 = NewNode('Acrylic', "ModuleScript", N1, function(script)
local Acrylic = {
...
local N3 = NewNode('AcrylicBlur', "ModuleScript", N2, function(script)
...
local N4 = NewNode('AcrylicPaint', "ModuleScript", N2, function(script)
local Creator = require(script.Parent.Parent.Creator)
...
local N5 = NewNode('CreateAcrylic', "ModuleScript", N2, function(script)
...
local N6 = NewNode('Utils', "ModuleScript", N2, function(script)
...
local N7 = NewNode('Components', "Folder", N1, nil)
local N8 = NewNode('Assets', "ModuleScript", N7, function(script)
return {
...
local N9 = NewNode('Button', "ModuleScript", N7, function(script)
local Root = script.Parent.Parent
...
return function(Theme, Parent, DialogCheck)
	...
	Creator.PressBounce(Button.Frame, 0.94, 1)
...
local N10 = NewNode('Dialog', "ModuleScript", N7, function(script)
local UserInputService = game:GetService("UserInputService")
...
function Dialog:Create()
	...
	function NewDialog:Open()
		...
		RootTransparency(0)
		Scale(1, false, Creator.SpringPresets.Bounce)
	end
	...
	function NewDialog:Close()
		...
		TintTransparency(1)
		RootTransparency(1)
		Scale(1.1, false, Creator.SpringPresets.Smooth)
		NewDialog.Root.UIStroke:Destroy()
		NewDialog.Root.GlowStroke:Destroy()
		task.wait(0.15)
...
local N11 = NewNode('Element', "ModuleScript", N7, function(script)
local Root = script.Parent.Parent
local Flipper = require(Root.Packages.Flipper)
local Creator = require(Root.Creator)
local New = Creator.New
...
return function(Title, Desc, Parent, Hover)
	...
	Element.GlowBorder = New("UIStroke", {
		Transparency = 1,
		Thickness = 1.25,
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
	...
	Element.Sheen = New("Frame", {
		...
		New("UIGradient", {
			...
			ThemeTag = {
				Color = "CardSheen",
				Transparency = "CardSheenTransparency",
			},
		}),
	})

	-- Slot is the object that actually lives inside the tab/section's
	-- UIListLayout. It's invisible and just reserves the vertical space.
	-- Element.Frame lives INSIDE it and is free to slide left/right for the
	-- tab-swap entrance animation without UIListLayout snapping its
	-- Position back every frame (list layouts own the Position of their
	-- direct children, so anything we want to slide has to be one level
	-- deeper than the list).
	Element.Slot = New("Frame", {
	...
	-- Marks this Frame as an animatable widget so the tab-swap transition
	-- can find every Toggle/Button/Dropdown/etc. inside the incoming tab
	-- (including ones nested inside Sections) and stagger their entrance.
	Element.Frame:SetAttribute("FluentWidget", true)
	...
	local GlowMotor, SetGlow = Creator.SpringMotor(1, Element.GlowBorder, "Transparency", true)
	local ScaleObj, SetScale = Creator.PressBounce(Element.Frame, 0.988, 1)

	Creator.AddSignal(Element.Frame.MouseEnter, function()
		SetGlow(1 - Creator.GetThemeProperty("GlowStrength") * 0.55)
	end)
	Creator.AddSignal(Element.Frame.MouseLeave, function()
		SetGlow(1)
	end)
	Creator.AddSignal(Element.Frame.MouseButton1Down, function()
		SetGlow(1 - Creator.GetThemeProperty("GlowStrength"))
	end)
...
local N12 = NewNode('Notification', "ModuleScript", N7, function(script)
local Root = script.Parent.Parent
local Flipper = require(Root.Packages.Flipper)
local Creator = require(Root.Creator)
local Acrylic = require(Root.Acrylic)
...
function Notification:New(Config)
	...
	local _, SetCloseHover = Creator.SpringMotor(1, NewNotification.CloseButton, "BackgroundTransparency", true)
	...
	function NewNotification:Open()
		...
		RootMotor:setGoal({
			Scale = Spring(0, Creator.SpringPresets.Bounce),
			Offset = Spring(0, Creator.SpringPresets.Bounce),
		})

		if ProgressFill and Config.Duration then
			local ProgressMotor = Flipper.SingleMotor.new(1)
			ProgressMotor:onStep(function(Value)
				ProgressFill.Size = UDim2.fromScale(Value, 1)
			end)
			ProgressMotor:setGoal(Flipper.Linear.new(0, { velocity = 1 / Config.Duration }))
		end
	end
	...
	function NewNotification:Close()
		if not NewNotification.Closed then
			...
			task.spawn(function()
				RootMotor:setGoal({
					Scale = Spring(1, Creator.SpringPresets.Smooth),
					Offset = Spring(60, Creator.SpringPresets.Smooth),
				})
				task.wait(0.4)
...
local N13 = NewNode('Section', "ModuleScript", N7, function(script)
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
function TabModule:New(Title, Icon, Parent)
	...
	Creator.PressBounce(Tab.Frame, 0.97, 1)
...
-- fading in as it goes, then slightly overshoots past its resting spot and
...
local function CollectWidgets(Container)
	...
	for _, Descendant in ipairs(Container:GetDescendants()) do
		if Descendant:GetAttribute("FluentWidget") then
			table.insert(Widgets, Descendant)
		end
	end
	return Widgets
end
...
-- fully invisible back to whatever it was meant to be, without touching
-- anything it doesn't have.
local function FadeDescendantIn(Descendant, TweenInfoObj)
	if Descendant:IsA("GuiObject") and not Descendant:IsA("UIScale") then
		local Goal = Descendant.BackgroundTransparency
		if Goal < 1 then
			Descendant.BackgroundTransparency = 1
			TweenService:Create(Descendant, TweenInfoObj, { BackgroundTransparency = Goal }):Play()
		end
	end
	if Descendant:IsA("TextLabel") or Descendant:IsA("TextButton") or Descendant:IsA("TextBox") then
		local Goal = Descendant.TextTransparency
		if Goal < 1 then
			Descendant.TextTransparency = 1
			TweenService:Create(Descendant, TweenInfoObj, { TextTransparency = Goal }):Play()
		end
	end
	if Descendant:IsA("ImageLabel") or Descendant:IsA("ImageButton") then
		local Goal = Descendant.ImageTransparency
		if Goal < 1 then
			Descendant.ImageTransparency = 1
			TweenService:Create(Descendant, TweenInfoObj, { ImageTransparency = Goal }):Play()
		end
	end
	if Descendant:IsA("UIStroke") then
		local Goal = Descendant.Transparency
		if Goal < 1 then
			Descendant.Transparency = 1
			TweenService:Create(Descendant, TweenInfoObj, { Transparency = Goal }):Play()
		end
	end
end
...
function TabModule:SelectTab(Tab)
	...
	for _, TabObject in next, TabModule.Tabs do
		...
		Creator.OverrideTag(TabObject.Icon, { ImageColor3 = "Text" })
		Creator.OverrideTag(TabObject.Label, { TextColor3 = "Text" })
	end
	TabModule.Tabs[Tab].SetTransparency(0.89)
	TabModule.Tabs[Tab].Selected = true
	Creator.OverrideTag(TabModule.Tabs[Tab].Icon, { ImageColor3 = "GlowAccent" })
	Creator.OverrideTag(TabModule.Tabs[Tab].Label, { TextColor3 = "GlowAccent" })
...
local N15 = NewNode('Textbox', "ModuleScript", N7, function(script)
...
return function(Parent, Acrylic)
	...
	Textbox.FocusGlow = New("UIStroke", {
		Transparency = 1,
		Thickness = 1.25,
		ThemeTag = { Color = "GlowAccent" },
	})
	...
	local IndicatorWidthMotor = Flipper.SingleMotor.new(0)
	IndicatorWidthMotor:onStep(function(Value)
		Textbox.Indicator.Size = UDim2.new(0, Value, 0, 2)
	end)
	local _, SetFocusGlow = Creator.SpringMotor(1, Textbox.FocusGlow, "Transparency", true)
	...
	Creator.AddSignal(Textbox.Input.Focused, function()
		Update()
		IndicatorWidthMotor:setGoal(Flipper.Spring.new(Textbox.Frame.AbsoluteSize.X - 4, Creator.SpringPresets.Bounce))
		...
		Creator.OverrideTag(Textbox.Indicator, { BackgroundColor3 = "GlowAccent" })
	end)

	Creator.AddSignal(Textbox.Input.FocusLost, function()
		Update()
		IndicatorWidthMotor:setGoal(Flipper.Spring.new(0, Creator.SpringPresets.Snap))
...
local N16 = NewNode('TitleBar', "ModuleScript", N7, function(script)
local Root = script.Parent.Parent
...
return function(Config)
	...
	local function BarButton(Icon, Pos, Parent, Callback, IsClose)
		...
		Button.Frame = New("TextButton", {
			...
			Text = "",
			ThemeTag = {
				BackgroundColor3 = IsClose and "GlowAccent" or "Text",
			},
		}, {
			New("UICorner", {
		...
		local _, SetStroke = Creator.SpringMotor(1, Button.Frame.UIStroke, "Transparency", true)
		Creator.PressBounce(Button.Frame, 0.82, 1)

		AddSignal(Button.Frame.MouseEnter, function()
			SetTransparency(IsClose and 0.85 or 0.92)
			SetStroke(0.6)
		end)
		...
		AddSignal(Button.Frame.MouseButton1Down, function()
			SetTransparency(IsClose and 0.78 or 0.88)
		end)
		...
		New("Frame", {
			...
			New("Frame", {
				...
				New("ImageLabel", {
					...
					ThemeTag = { ImageColor3 = "GlowAccent" },
				}),
			}),
			New("TextLabel", {
				RichText = true,
	...
	TitleBar.MaxButton = BarButton(Assets.Max, UDim2.new(1, -40, 0.5, 0), TitleBar.Frame, function()
		Config.Window.Maximize(not Config.Window.Maximized)
	end)
	TitleBar.MinButton = BarButton(Assets.Min, UDim2.new(1, -72, 0.5, 0), TitleBar.Frame, function()
...
local N17 = NewNode('Window', "ModuleScript", N7, function(script)
local UserInputService = game:GetService("UserInputService")
local Mouse = game:GetService("Players").LocalPlayer:GetMouse()
local Camera = game:GetService("Workspace").CurrentCamera
...
return function(Config)
	...
	local SelectorGlow = New("ImageLabel", {
		Image = "rbxassetid://5028857084",
		ScaleType = Enum.ScaleType.Slice,
		SliceCenter = Rect.new(20, 20, 280, 280),
		AnchorPoint = Vector2.new(0, 0.5),
	...
	do
		local EntranceMotor = Flipper.SingleMotor.new(0.9)
		EntranceMotor:onStep(function(Value)
			EntranceScale.Scale = Value
		end)
		EntranceMotor:setGoal(Flipper.Spring.new(1, Creator.SpringPresets.Bounce))
	end
	...
	Window.SelectorPosMotor:onStep(function(Value)
		...
		if LastValue ~= nil then
			Window.SelectorSizeMotor:setGoal(
				Spring((math.abs(Value - LastValue) / (DeltaTime * 60)) + 16, Creator.SpringPresets.Bounce)
			)
			LastValue = Value
		end
		LastTime = Now
	...
	Window.Maximize = function(Value, NoPos, Instant)
		...
		if not NoPos then
			PosMotor:setGoal({
				X = Spring(Value and 0 or Window.Position.X.Offset, Creator.SpringPresets.Smooth),
				Y = Spring(Value and 0 or Window.Position.Y.Offset, Creator.SpringPresets.Smooth),
			})
		end
	end
...
local N18 = NewNode('Creator', "ModuleScript", N1, function(script)
...
local Creator = {
	...
	SnapInfo = TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
	SmoothInfo = TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
	DefaultProperties = {
		ScreenGui = {
			ResetOnSpawn = false,
...
-- Applies theme-driven properties to a SINGLE object only. This is the
-- cheap, local counterpart to Creator.UpdateTheme() (which re-applies colors
-- to EVERY themed object in the whole UI and hard-resets every transparency
-- motor to Instant). Use this whenever only one object's tags changed.
function Creator.ApplyThemeProperties(Object, Properties)
...
function Creator.AddThemeObject(Object, Properties)
	...
	-- NOTE: this used to call the full Creator.UpdateTheme(), which loops
	-- through every themed object that has EVER been created (hundreds of
	-- them once a real window is built) and also snaps every hover/press
	-- transparency motor straight to Instant. Since new themed objects are
	-- created constantly during normal use (button ripples, dropdown
	-- options, tab swaps, etc.), that meant clicking almost anything caused
	-- a global "flash reset" of every other element's hover/glow/border
	-- animation on screen - the stutter/disappearing-border bug. We only
	-- need to color THIS new object; nothing else has changed.
	Creator.ApplyThemeProperties(Object, Properties)
	return Object
end

function Creator.OverrideTag(Object, Properties)
	Creator.Registry[Object].Properties = Properties
	-- Same reasoning as above: only this object's tags changed, so only
	-- re-color this object instead of the whole registry + motor reset.
	Creator.ApplyThemeProperties(Object, Properties)
end
...
function Creator.SpringMotor(Initial, Instance, Prop, IgnoreDialogCheck, ResetOnThemeChange, Transform)
	IgnoreDialogCheck = IgnoreDialogCheck or false
	ResetOnThemeChange = ResetOnThemeChange or false
	local Motor = Flipper.SingleMotor.new(Initial)
	Motor:onStep(function(value)
		Instance[Prop] = Transform and Transform(value) or value
	end)
	...
	local function SetValue(Value, Ignore, Preset)
		...
		Motor:setGoal(Flipper.Spring.new(Value, Preset or Creator.SpringPresets.Snap))
	end

	return Motor, SetValue
end

function Creator.AddGlow(Parent, Size, BaseTransparency)
	Size = Size or UDim2.new(1, 40, 1, 40)
	BaseTransparency = BaseTransparency or 1
...
function Creator.PressBounce(Button, PressScale, RestScale)
	PressScale = PressScale or 0.965
	RestScale = RestScale or 1

	local Scale = Creator.New("UIScale", { Scale = RestScale })
	Scale.Parent = Button

	local Motor = Flipper.SingleMotor.new(RestScale)
	Motor:onStep(function(Value)
		Scale.Scale = Value
	end)

	local function SetScale(Value, Preset)
		Motor:setGoal(Flipper.Spring.new(Value, Preset or Creator.SpringPresets.Bounce))
	end

	Creator.AddSignal(Button.MouseButton1Down, function()
		SetScale(PressScale, Creator.SpringPresets.Pop)
	end)
	Creator.AddSignal(Button.MouseButton1Up, function()
		SetScale(RestScale, Creator.SpringPresets.Bounce)
	end)
...
local N19 = NewNode('Elements', "ModuleScript", N1, function(script)
local Elements = {}
...
local N20 = NewNode('Button', "ModuleScript", N19, function(script)
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
function Element:New(Config)
	...
	local _, SetIconPos = Creator.SpringMotor(-10, ButtonIco, "Position", true, false, function(Value)
		return UDim2.new(1, Value, 0.5, 0)
	end)

	Creator.AddSignal(ButtonFrame.Frame.MouseEnter, function()
		SetIconPos(-7, Creator.SpringPresets.Bounce)
		Creator.OverrideTag(ButtonIco, { ImageColor3 = "GlowAccent" })
	end)
	Creator.AddSignal(ButtonFrame.Frame.MouseLeave, function()
		SetIconPos(-10, Creator.SpringPresets.Bounce)
		Creator.OverrideTag(ButtonIco, { ImageColor3 = "Text" })
	end)
	...
	Creator.AddSignal(ButtonFrame.Frame.MouseButton1Click, function()
		...
		local RippleMotor, SetRipple = Creator.SpringMotor(10, Ripple, "Size", true, false, function(Value)
			return UDim2.fromOffset(Value, Value)
		end)
		local _, SetRippleFade = Creator.SpringMotor(0.35, Ripple, "ImageTransparency", true)
		SetRipple(240, Creator.SpringPresets.Snap)
		SetRippleFade(1, Creator.SpringPresets.Snap)
...
local N21 = NewNode('Colorpicker', "ModuleScript", N19, function(script)
local UserInputService = game:GetService("UserInputService")
...
function Element:New(Idx, Config)
	...
	local _, SetSwatchGlow = Creator.SpringMotor(1, SwatchGlow, "Transparency", true)
	Creator.AddSignal(ColorpickerFrame.Frame.MouseEnter, function()
		SetSwatchGlow(0.35)
	end)
	Creator.AddSignal(ColorpickerFrame.Frame.MouseLeave, function()
		SetSwatchGlow(1)
	end)
...
local N22 = NewNode('Dropdown', "ModuleScript", N19, function(script)
local TweenService = game:GetService("TweenService")
local UserInputService = game:GetService("UserInputService")
local Mouse = game:GetService("Players").LocalPlayer:GetMouse()
local Camera = game:GetService("Workspace").CurrentCamera
	...
	function Dropdown:BuildDropdownList()
		...
		for Idx, Value in next, Values do
			...
			function Table:UpdateButton()
				...
				SelectorSizeMotor:setGoal(Flipper.Spring.new(Selected and 14 or 6, Creator.SpringPresets.Bounce))
...
local N23 = NewNode('Input', "ModuleScript", N19, function(script)
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
local N24 = NewNode('Keybind', "ModuleScript", N19, function(script)
local UserInputService = game:GetService("UserInputService")
...
function Element:New(Idx, Config)
	...
	local KeybindGlow = New("UIStroke", {
		Transparency = 1,
		Thickness = 1.25,
		ThemeTag = { Color = "GlowAccent" },
	})
	...
	local _, SetKeybindGlow = Creator.SpringMotor(1, KeybindGlow, "Transparency", true)
	local ListeningLoop = false
	...
	Creator.AddSignal(KeybindDisplayFrame.InputBegan, function(Input)
		...
		then
			...
			Creator.OverrideTag(KeybindDisplayLabel, { TextColor3 = "GlowAccent" })

			ListeningLoop = true
			task.spawn(function()
				while ListeningLoop do
					SetKeybindGlow(0.15, Creator.SpringPresets.Glow)
					task.wait(0.55)
					if not ListeningLoop then
						break
					end
					SetKeybindGlow(0.75, Creator.SpringPresets.Glow)
					task.wait(0.55)
				end
			end)
			...
			Event = UserInputService.InputBegan:Connect(function(Input)
				...
				EndedEvent = UserInputService.InputEnded:Connect(function(Input)
					...
					then
						Picking = false
						ListeningLoop = false
						Creator.OverrideTag(KeybindDisplayLabel, { TextColor3 = "Text" })
						SetKeybindGlow(1, Creator.SpringPresets.Pop)
...
local N25 = NewNode('Paragraph', "ModuleScript", N19, function(script)
local Root = script.Parent.Parent
local Components = Root.Components
local Flipper = require(Root.Packages.Flipper)
local Creator = require(Root.Creator)
...
local N26 = NewNode('Slider', "ModuleScript", N19, function(script)
local UserInputService = game:GetService("UserInputService")
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
local Flipper = require(Root.Packages.Flipper)
...
function Element:New(Idx, Config)
	...
	local SliderDot = New("ImageLabel", {
		...
		DotGlow,
		New("UIScale", { Scale = 1, Name = "DotScale" }),
	})
	local DotScale = SliderDot.DotScale
	...
	local _, SetGlow = Creator.SpringMotor(0.55, DotGlow, "ImageTransparency", true)

	local function SetFraction(Fraction, Instant)
		if Instant then
			FractionMotor:setGoal(Flipper.Instant.new(Fraction))
		else
			FractionMotor:setGoal(Flipper.Spring.new(Fraction, Creator.SpringPresets.Bounce))
		end
	end
	...
	Creator.AddSignal(SliderDot.InputBegan, function(Input)
		...
		then
			...
			ScaleMotor:onStep(function(v)
				DotScale.Scale = v
			end)
			ScaleMotor:setGoal(Flipper.Spring.new(1.35, Creator.SpringPresets.Pop))
			SetGlow(0.1)
		end
	end)
	...
	Creator.AddSignal(SliderDot.InputEnded, function(Input)
		...
		then
			Dragging = false
			local ScaleMotor = Flipper.SingleMotor.new(DotScale.Scale)
			ScaleMotor:onStep(function(v)
				DotScale.Scale = v
			end)
			ScaleMotor:setGoal(Flipper.Spring.new(1, Creator.SpringPresets.Bounce))
			SetGlow(0.55)
		end
	end)
	...
	Creator.AddSignal(UserInputService.InputChanged, function(Input)
		...
		then
			...
			Slider:SetValue(Slider.Min + ((Slider.Max - Slider.Min) * SizeScale), true)
		end
	end)
	...
	function Slider:SetValue(Value, IsDragging)
		self.Value = Library:Round(math.clamp(Value, Slider.Min, Slider.Max), Slider.Rounding)
		SetFraction((self.Value - Slider.Min) / (Slider.Max - Slider.Min), IsDragging)
...
local N27 = NewNode('Toggle', "ModuleScript", N19, function(script)
local TweenService = game:GetService("TweenService")
local Root = script.Parent.Parent
local Creator = require(Root.Creator)
...
function Element:New(Idx, Config)
	...
	ToggleFrame.DescLabel.Size = UDim2.new(1, -60, 0, 14)
	...
	local ToggleCircle = New("ImageLabel", {
		AnchorPoint = Vector2.new(0, 0.5),
		Size = UDim2.fromOffset(15, 15),
		Position = UDim2.new(0, 2.5, 0.5, 0),
		Image = "http://www.roblox.com/asset/?id=12266946128",
	...
	local _, SetToggleGlow = Creator.SpringMotor(1, ToggleGlow, "ImageTransparency", true)
	...
	function Toggle:SetValue(Value)
		...
		TweenService:Create(
			ToggleCircle,
			Creator.BounceInfo,
			{ Position = UDim2.new(0, Toggle.Value and 20.5 or 2.5, 0.5, 0) }
		):Play()
		TweenService:Create(
			ToggleSlider,
			Creator.SmoothInfo,
		...
		if Toggle.Value then
			task.delay(0.18, function()
				if Toggle.Value then
					SetToggleGlow(0.75, Creator.SpringPresets.Glow)
				end
			end)
		end
...
local N29 = NewNode('Packages', "Folder", N1, nil)
local N30 = NewNode('Flipper', "ModuleScript", N29, function(script)
local Flipper = {
...
local N31 = NewNode('BaseMotor', "ModuleScript", N30, function(script)
local RunService = game:GetService("RunService")
...
local N32 = NewNode('BaseMotor.spec', "ModuleScript", N30, function(script)
return function()
	local RunService = game:GetService("RunService")
...
local N33 = NewNode('GroupMotor', "ModuleScript", N30, function(script)
local BaseMotor = require(script.Parent.BaseMotor)
...
local N34 = NewNode('GroupMotor.spec', "ModuleScript", N30, function(script)
return function()
...
local N35 = NewNode('Instant', "ModuleScript", N30, function(script)
local Instant = {}
...
local N36 = NewNode('Instant.spec', "ModuleScript", N30, function(script)
return function()
	local Instant = require(script.Parent.Instant)
...
local N37 = NewNode('Linear', "ModuleScript", N30, function(script)
local Linear = {}
...
local N38 = NewNode('Linear.spec', "ModuleScript", N30, function(script)
return function()
	local SingleMotor = require(script.Parent.SingleMotor)
...
local N39 = NewNode('Signal', "ModuleScript", N30, function(script)
local Connection = {}
...
local N40 = NewNode('Signal.spec', "ModuleScript", N30, function(script)
return function()
	local Signal = require(script.Parent.Signal)
...
local N41 = NewNode('SingleMotor', "ModuleScript", N30, function(script)
local BaseMotor = require(script.Parent.BaseMotor)
...
local N42 = NewNode('SingleMotor.spec', "ModuleScript", N30, function(script)
return function()
	local SingleMotor = require(script.Parent.SingleMotor)
	local Instant = require(script.Parent.Instant)
...
local N43 = NewNode('Spring', "ModuleScript", N30, function(script)
...
local N44 = NewNode('Spring.spec', "ModuleScript", N30, function(script)
return function()
	local SingleMotor = require(script.Parent.SingleMotor)
	local Spring = require(script.Parent.Spring)
...
local N45 = NewNode('isMotor', "ModuleScript", N30, function(script)
...
local N46 = NewNode('isMotor.spec', "ModuleScript", N30, function(script)
return function()
	local isMotor = require(script.Parent.isMotor)
...
local N47 = NewNode('Themes', "ModuleScript", N1, function(script)
local Themes = {
	Names = {
		"Pepper Gray",
		"Dark",
...
local N48 = NewNode('Amethyst', "ModuleScript", N47, function(script)
return {
	Name = "Amethyst",
...
local N49 = NewNode('Aqua', "ModuleScript", N47, function(script)
return {
	Name = "Aqua",
...
local N50 = NewNode('Dark', "ModuleScript", N47, function(script)
return {
	Name = "Dark",
	...
	GlowAccent = Color3.fromRGB(150, 190, 255),
	GlowSoft = Color3.fromRGB(255, 255, 255),
	GlowStrength = 0.82,
	ShadowColor = Color3.fromRGB(0, 0, 0),
	CardSheen = ColorSequence.new(Color3.fromRGB(255, 255, 255), Color3.fromRGB(255, 255, 255)),
	CardSheenTransparency = NumberSequence.new(0.9, 1),
}
end)
local N51 = NewNode('Darker', "ModuleScript", N47, function(script)
return {
	Name = "Darker",
...
local N52 = NewNode('Light', "ModuleScript", N47, function(script)
return {
	Name = "Light",
...
local N53 = NewNode('PepperGray', "ModuleScript", N47, function(script)
return {
	Name = "Pepper Gray",
	Accent = Color3.fromRGB(176, 193, 214),

	AcrylicMain = Color3.fromRGB(30, 30, 33),
	AcrylicBorder = Color3.fromRGB(96, 101, 110),
	AcrylicGradient = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(48, 49, 54)),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(34, 34, 38)),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(24, 24, 27)),
	}),
	AcrylicNoise = 0.95,

	TitleBarLine = Color3.fromRGB(72, 74, 80),
	Tab = Color3.fromRGB(150, 155, 165),

	Element = Color3.fromRGB(150, 155, 165),
	ElementBorder = Color3.fromRGB(54, 55, 60),
	InElementBorder = Color3.fromRGB(100, 104, 112),
	ElementTransparency = 0.85,

	ToggleSlider = Color3.fromRGB(142, 146, 154),
	ToggleToggled = Color3.fromRGB(16, 17, 19),

	SliderRail = Color3.fromRGB(142, 146, 154),

	DropdownFrame = Color3.fromRGB(178, 183, 193),
	DropdownHolder = Color3.fromRGB(39, 39, 43),
	DropdownBorder = Color3.fromRGB(52, 53, 58),
	DropdownOption = Color3.fromRGB(142, 146, 154),

	Keybind = Color3.fromRGB(142, 146, 154),

	Input = Color3.fromRGB(178, 183, 193),
	InputFocused = Color3.fromRGB(15, 15, 17),
	InputIndicator = Color3.fromRGB(168, 173, 183),

	Dialog = Color3.fromRGB(37, 37, 41),
	DialogHolder = Color3.fromRGB(29, 29, 32),
	DialogHolderLine = Color3.fromRGB(24, 24, 27),
	DialogButton = Color3.fromRGB(37, 37, 41),
	DialogButtonBorder = Color3.fromRGB(90, 90, 98),
	DialogBorder = Color3.fromRGB(80, 84, 92),
	DialogInput = Color3.fromRGB(45, 45, 49),
	DialogInputLine = Color3.fromRGB(176, 193, 214),

	Text = Color3.fromRGB(239, 240, 243),
	SubText = Color3.fromRGB(160, 163, 172),
	...
	GlowSoft = Color3.fromRGB(255, 255, 255),
	GlowStrength = 0.9,
	ShadowColor = Color3.fromRGB(4, 4, 5),
	CardSheen = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 255, 255)),
		ColorSequenceKeypoint.new(0.4, Color3.fromRGB(210, 216, 224)),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(180, 186, 196)),
	}),
	CardSheenTransparency = NumberSequence.new({
		NumberSequenceKeypoint.new(0, 0.88),
		NumberSequenceKeypoint.new(1, 1),
	}),
}
end)
local N54 = NewNode('Rose', "ModuleScript", N47, function(script)
return {
	Name = "Rose",
